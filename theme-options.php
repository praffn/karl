<?php
	...
	function YourTheme_settings_hero_image() {
		$options = YourTheme_get_theme_options();
		?>
			<input type="text" name="YourTheme_theme_options[hero_image]" id="hero_image" value="<?php echo esc_attr($options['hero_image']); ?>">
			<label class="description" for="hero_image">The big image that will appear on the front page's hero section</label>
		<?php
	}

	function YourTheme_settings_hero_cta_label() {
		$options = YourTheme_get_theme_options();
		?>
			<input type="text" name="YourTheme_theme_options[hero_cta_label]" id="hero_cta_label" value="<?php echo esc_attr($options['hero_cta_label']); ?>">
			<label class="description" for="hero_cta_label">The label for the Call-to-Action button on the hero</label>
		<?php
	}

	function YourTheme_settings_hero_cta_link() {
		$options = YourTheme_get_theme_options();
		?>
			<input type="text" name="YourTheme_theme_options[hero_cta_link]" id="hero_cta_link" value="<?php echo esc_attr($options['hero_cta_link']); ?>">
			<label class="description" for="hero_cta_link">The URL to the page that the call-to-action should lead to</label>
		<?php
	}



	function YourTheme_settings_hero_title() {
    $options = YourTheme_get_theme_options();
    ?>
    <input type="text" name="YourTheme_theme_options[hero_title]" id="hero_title" value="<?php echo esc_attr( $options['hero_title'] ); ?>" />
    <label class="description" for="hero_title">This title will appear on the hero image on the front page</label>
    <?php
	}

	function YourTheme_settings_hero_subtitle() {
    $options = YourTheme_get_theme_options();
    ?>
    <input type="text" name="YourTheme_theme_options[hero_subtitle]" id="hero_subtitle" value="<?php echo esc_attr( $options['hero_subtitle'] ); ?>" />
    <label class="description" for="hero_subtitle">This subtitle will appear on the hero image on the front page</label>
    <?php
	}
 ...
 */
	...
	function YourTheme_theme_options_validate( $input ) {
		...
		if ( isset( $input['hero_title'] ) && ! empty( $input['hero_title'] ) )
    	$output['hero_title'] = wp_filter_nohtml_kses( $input['hero_title'] );

    if ( isset( $input['hero_image'] ) && ! empty( $input['hero_image'] ) )
    	$output['hero_image'] = wp_filter_nohtml_kses( $input['hero_image'] );

    if ( isset( $input['hero_subtitle'] ) && ! empty( $input['hero_subtitle'] ) )
    	$output['hero_subtitle'] = wp_filter_nohtml_kses( $input['hero_subtitle'] );

    if ( isset( $input['hero_cta_label'] ) && ! empty( $input['hero_cta_label'] ) )
    	$output['hero_cta_label'] = wp_filter_nohtml_kses( $input['hero_cta_label'] );

    if ( isset( $input['hero_cta_link'] ) && ! empty( $input['hero_cta_link'] ) )
    	$output['hero_cta_link'] = wp_filter_nohtml_kses( $input['hero_cta_link'] );
	...
	function YourTheme_theme_options_init() {
	    ...
	    'hero', // Unique identifier for the settings section
	    'Hero', // Section title (we don't want one)
		...
		add_settings_field( 'hero_title', 'Hero Title', 'YourTheme_settings_hero_title', 'theme_options', 'hero' );
		add_settings_field('hero_subtitle', 'Hero Subtitle', 'YourTheme_settings_hero_subtitle', 'theme_options', 'hero');
		add_settings_field('hero_image', 'Hero Image', 'YourTheme_settings_hero_image', 'theme_options', 'hero');
		add_settings_field('hero_cta_label', 'Call-to-Action Label', 'YourTheme_settings_hero_cta_label', 'theme_options', 'hero');
		add_settings_field('hero_cta_link', 'Call-to-Action Link', 'YourTheme_settings_hero_cta_link', 'theme_options', 'hero');
